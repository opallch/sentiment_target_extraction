...
class FeatureVectorCreator:
    ...
    def get_vectors(self) -> None:
        """
        wrapper function which contains all the functionalities: 
        creates features vectors, saves them in a dataframe and write them into a `.pkl` file.
        """
        self._all_features_for_all_instances()
        self._write_vectors_to_file()
    
    def _all_features_for_all_instances(self) -> None:
        """appends the features lists for all instances in `self._list_of_vecs`"""
    ...
    def _append_vector_to_list(self, features_vec:list) -> list:
        """appends the features lists for an instance in `self._list_of_vecs`"""
        self._list_of_vecs.append(features_vec)

    def _all_features_for_each_instance(self, df_row:pd.Series) -> list:
        """combines all features vectors from different feature classes into one single vector aka. list for an instance."""
        all_vectors = []
        
        for features_class in self._features_classes:
        
            all_vectors.append(
                features_class.get_features(df_row)
                )
       
        return [f for vec in all_vectors for f in vec]

    def _write_vectors_to_file(self) -> None:
        """writes self.df_vectors to the ouput `.pkl` file."""
        pd.to_pickle(
    ...
    def _list_of_vecs2df(self) -> pd.DataFrame:
        """turns `self._list_of_vecs` into a dataframe."""
        return pd.DataFrame(
            np.array(self._list_of_vecs)
        )
...
if __name__ == "__main__":
    ...
    print(
        pd.read_pickle("../test_files/test_all_features.pkl")[:10]
    )